  ...
  <div class="search">
    ...
    >
      ...
      <el-form-item  prop="searchEmail">
        <el-input v-model="state.form.searchEmail" placeholder="邮箱"/>
      </el-form-item>
      <el-form-item>
        <el-button type="primary" :icon="Search" @click="methods.handleCurrentChange(1)">查询</el-button>
        <el-button type="primary" :icon="Refresh" @click="state.form.searchUsername='';state.form.searchEmail=''">重置</el-button>
      </el-form-item>
    </el-form>
  </div>
      ...
      <el-table-column prop="avatar" align="center" label="头像">
        <template #default="scope">
          <el-tooltip
              ...
              placement="right"
          >
            <el-avatar shape="square" :size="35" :src="axios.defaults.baseURL+'/images?username='+scope.row.username+'&spec=120'" @click="methods.openAvatarDialog(scope.row)"/>
          </el-tooltip>
        </template>
      </el-table-column>
  ...
  setup(){
    ...
    const methods={
        ...
        proxy.$refs['form'].validate(valid => {
          if(valid){
            searchUser({
              ...
              usertype:state.form.searchEmail,
              page
            }).then(res=>{
              if(res.data.status){
                state.loading = false;
      ...
      handleCurrentChange:(page)=>{
        if(state.form.searchUsername===''&&state.form.searchEmail===''){
          getLists(page);
        }else{
          methods.search(page);
        }
